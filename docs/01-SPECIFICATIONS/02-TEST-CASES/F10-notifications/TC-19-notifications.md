...
| 3 | Verify content | Title "Ülesanne loodud!" visible | ☐ |
...
| # | Action | Expected Result | Pass |
|---|--------|-----------------|------|
| 1 | Trigger an error (e.g., stop backend) | Error occurs | ☐ |
| 2 | Observe notification | Red error notification appears | ☐ |
| 3 | Verify content | Error message visible | ☐ |

### Notification Types

Test each type is visually distinct:

| Type | Trigger Action | Expected Color |
|------|----------------|----------------|
| Success | Create task | Green |
| Error | API failure | Red |
| Info | Copy link | Blue |
| Warning | (if applicable) | Yellow |
...
| Task updated | Success: "Ülesanne uuendatud!" |
...
- Action buttons navigate without closing explicitly
