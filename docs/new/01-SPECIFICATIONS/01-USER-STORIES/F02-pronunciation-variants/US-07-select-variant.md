...
- [x] **AC-1:** Each variant has a "Kasuta" (Use) button
- [x] **AC-2:** Clicking "Kasuta" applies the variant's phonetic form to the sentence
- [x] **AC-3:** The original word in the tag display remains unchanged
- [x] **AC-4:** The phonetic form (stressedTags) is updated with the variant
- [x] **AC-5:** Audio cache is invalidated to force re-synthesis
- [x] **AC-6:** Selected variant is highlighted in the panel
- [x] **AC-7:** Panel remains open after selection (for further exploration)
- [x] **AC-8:** Next synthesis uses the selected variant's phonetic form
...
3. Variant item shows selected state (highlighted with different background)
...
- Selected variant shows visual highlight (different background color)
- Previously selected variant loses highlight when new one selected
- "Kasuta" button text remains the same (no state change to "Valitud")
