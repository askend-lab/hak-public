...
test.describe('TARA Authentication', () => {
  ...
  test('TARA start endpoint redirects to TARA login page', async ({ request }) => {
    const response = await request.get('https://hak-api-dev.askend-lab.com/auth/tara/start', {
      maxRedirects: 0,
    });
  ...
  test('TARA callback handles missing code parameter', async ({ request }) => {
    const response = await request.get('https://hak-api-dev.askend-lab.com/auth/tara/callback', {
      maxRedirects: 0,
    });
  ...
  test('Login modal shows TARA button and redirects on click', async ({ page }) => {
    await page.goto('/');

    // Open login modal by clicking login button
    const loginButton = page.locator('button, a').filter({ hasText: /logi sisse|login/i }).first();
    ...
    if (await taraButton.isVisible({ timeout: 5000 })) {
      ...
      const navigationPromise = page.waitForURL(/tara-test\.ria\.ee|hak-api.*auth\/tara/, { 
        timeout: 10000,
        waitUntil: 'commit'
      });
  ...
  test('TARA authorization URL contains required OIDC parameters', async ({ request }) => {
    const response = await request.get('https://hak-api-dev.askend-lab.com/auth/tara/start', {
      maxRedirects: 0,
    });
