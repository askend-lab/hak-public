...
const SCSS_RULES = [
  {
    ...
    suggestion:
      "Replace with $color-* token (e.g., $color-primary, $color-white)",
  ...
  },
  {
    ...
    description:
      'Use $color-black or appropriate token instead of "black" keyword',
    severity: SEVERITY.HIGH,
    pattern: /(?<!\/\/.*?)(?<!\/\*.*?):\s*black\s*[;,)]/g,
  ...
  },
  {
    ...
    suggestion:
      "Use @include respond-below($breakpoint-*) or $breakpoint-* variable",
...
const REACT_RULES = [
  ...
  },
  {
    ...
    pattern:
      /className\s*=\s*["'`]([A-Z][a-zA-Z]+|[a-z]+[A-Z][a-zA-Z]*)\s*["'`]/g,
...
function checkMissingTokenImports(content, filePath) {
  ...
  const tokenPatterns = [
    {
      usage: /\$color-/g,
      import: "@import '../tokens/colors'",
      token: "colors",
    },
    {
      usage: /\$spacing-/g,
      import: "@import '../tokens/spacing'",
      token: "spacing",
    },
    {
      usage: /\$font-(?:size|weight|family)/g,
      import: "@import '../tokens/typography'",
      token: "typography",
    },
    {
      usage: /\$line-height-/g,
    ...
    {
      usage: /\$letter-spacing-/g,
      import: "@import '../tokens/typography'",
      token: "typography",
    },
    {
      usage: /\$border-radius/g,
      import: "@import '../tokens/borders'",
      token: "borders",
    },
    {
      usage: /\$border-width/g,
      import: "@import '../tokens/borders'",
      token: "borders",
    },
    {
      usage: /\$breakpoint-/g,
      import: "@import '../tokens/breakpoints'",
      token: "breakpoints",
    },
    {
      usage: /\$opacity-/g,
      import: "@import '../tokens/opacity'",
      token: "opacity",
    },
  ];
  ...
  for (const { usage, import: importPath, token } of tokenPatterns) {
    ...
    if (usageMatch && usageMatch.length > 0) {
      ...
      if (!hasImport) {
        ...
        const reliesOnCascade =
          /Token imports removed|using global imports|imports from main\.scss/i.test(
            content,
          );
  ...
  for (const rule of SCSS_RULES) {
    ...
    if (rule.customCheck) {
      ...
      for (const v of violations) {
        ...
        const effectiveSeverity = v.reliesOnCascade
          ? SEVERITY.MEDIUM
          : rule.severity;
        addViolation(
          rule.id,
...
function printResults() {
  ...
  console.log(
    `   Files with issues:    ${results.summary.filesWithViolations}`,
  );
  ...
  console.log("─".repeat(60));
  const sortedTypes = Object.entries(results.summary.byType).sort(
    (a, b) => b[1] - a[1],
  );
  ...
  if (topFiles.length > 0) {
    ...
    for (const [file, count] of topFiles) {
      const severityIcon =
        results.violations.find((v) => v.file === file)?.severity === "critical"
          ? "🔴"
          : "⚠️";
  ...
  if (highViolations.length > 0) {
    ...
    if (highViolations.length > 10) {
      console.log(
        `   ... and ${highViolations.length - 10} more high violations\n`,
      );
    }
    console.log("");
  }
  ...
  if (passed) {
    ...
    console.log(
      "❌ VALIDATION FAILED - Critical/High severity issues must be fixed\n",
    );
  }
