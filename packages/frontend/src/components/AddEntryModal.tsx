...
interface AddEntryModalProps { isOpen: boolean; onClose: () => void; onAdd: (title: string, description: string) => Promise<void>; }
...
export default function AddEntryModal({ isOpen, onClose, onAdd }: AddEntryModalProps) {
  ...
  const resetForm = () => { setTitle(''); setDescription(''); setError(null); };
  const handleClose = () => { if (!isSubmitting) { resetForm(); onClose(); } };
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!title.trim()) { setError('Pealkiri on kohustuslik'); return; }
    setIsSubmitting(true); setError(null);
    try { await onAdd(title.trim(), description.trim()); resetForm(); onClose(); }
    catch (err) { setError(err instanceof Error ? err.message : 'Viga ülesande lisamisel'); }
    finally { setIsSubmitting(false); }
  };
  if (!isOpen) return null;
  return <BaseModal isOpen={isOpen} onClose={handleClose} title="Lisa ülesanne" size="medium" className="add-entry-modal"><AddEntryForm title={title} description={description} isSubmitting={isSubmitting} error={error} onTitleChange={setTitle} onDescChange={setDescription} onSubmit={handleSubmit} /></BaseModal>;
}
