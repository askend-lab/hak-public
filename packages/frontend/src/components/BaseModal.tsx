...
interface BaseModalProps { isOpen: boolean; onClose: () => void; title?: string; showCloseButton?: boolean; size?: 'small' | 'medium' | 'large'; children: React.ReactNode; className?: string; headerClassName?: string; contentClassName?: string; preventBackdropClose?: boolean; }

const ModalHeader = ({ title, showCloseButton, onClose, headerClasses }: { title?: string; showCloseButton: boolean; onClose: () => void; headerClasses: string }) => (
  ...
  return (
    <>
      ...
      <div className={modalClasses}>
        {(title || showCloseButton) && <ModalHeader title={title} showCloseButton={showCloseButton} onClose={onClose} headerClasses={`base-modal__header ${headerClassName}`.trim()} />}
