...
const ModalHeader = ({ title, showCloseButton, onClose, headerClasses, titleId }: { title?: string | null | undefined; showCloseButton: boolean; onClose: () => void; headerClasses: string; titleId: string }) => (
  <div className={headerClasses}>
    {title && <h2 id={titleId} className="base-modal__title">{title}</h2>}
...
export default function BaseModal({ isOpen, onClose, title, showCloseButton = true, size = 'medium', children, className = '', headerClassName = '', contentClassName = '', preventBackdropClose = false }: BaseModalProps) {
  ...
  return (
    <>
      <div className="base-modal__backdrop" onClick={handleBackdropClick} aria-hidden="true" />
      <div 
        ref={modalRef}
      ...
      >
        {(title !== null || showCloseButton) && <ModalHeader title={title} showCloseButton={showCloseButton} onClose={onClose} headerClasses={`base-modal__header ${headerClassName}`.trim()} titleId={titleId} />}
