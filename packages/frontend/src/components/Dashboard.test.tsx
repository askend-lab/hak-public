...
vi.mock('../contexts/AuthContext', () => ({
  useAuth: vi.fn(() => ({ user: null, isAuthenticated: false, isLoading: false, showLoginModal: false, setShowLoginModal: vi.fn(), login: vi.fn(), logout: vi.fn() }))
}));

vi.mock('../services/dataService', () => ({
  DataService: { getInstance: vi.fn(() => ({ getUserTasks: vi.fn().mockResolvedValue([]) })) }
}));
...
describe('Dashboard', () => {
  ...
  it('renders metrics cards', async () => {
    vi.mocked(useAuth).mockReturnValue({ user: { id: '1', email: 't@t.com', name: 'T' }, isAuthenticated: true, isLoading: false, showLoginModal: false, setShowLoginModal: vi.fn(), login: vi.fn(), logout: vi.fn() });
    render(<Dashboard />);
  ...
  it('renders recent activity section', async () => {
    render(<Dashboard />);
    await waitFor(() => expect(screen.getByText('Hiljutine tegevus')).toBeTruthy());
  });
});
