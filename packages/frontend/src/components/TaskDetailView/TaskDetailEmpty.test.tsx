...
describe('TaskDetailEmpty', () => {
  ...
  it('renders placeholder text when task is null', () => {
    ...
    expect(screen.getByText('Alusta sisu loomisega, et lisada lauseid sellesse ülesandesse.')).toBeInTheDocument();
  });

  it('renders default description when task has no description', () => {
    const taskWithoutDesc = { ...mockTask, description: '' };
    render(<TaskDetailEmpty task={taskWithoutDesc} onNavigateToSynthesis={vi.fn()} />);
    expect(screen.getByText('Test')).toBeInTheDocument();
    expect(screen.getByText('Alusta sisu loomisega, et lisada lauseid sellesse ülesandesse.')).toBeInTheDocument();
  });
  ...
  it('calls onNavigateToSynthesis when button clicked', async () => {
    ...
    render(<TaskDetailEmpty task={mockTask} onNavigateToSynthesis={onNavigateToSynthesis} />);
  ...
  it('uses design system empty-state classes', () => {
    const { container } = render(<TaskDetailEmpty task={mockTask} onNavigateToSynthesis={vi.fn()} />);
