...
describe('useDragAndDrop', () => {
  ...
  it('sets draggedId on drag start', () => {
    const { result } = renderHook(() => useDragAndDrop(vi.fn()));
    const mockEvent = { dataTransfer: { effectAllowed: '', setData: vi.fn() } } as unknown as React.DragEvent;
    act(() => { result.current.handleDragStart(mockEvent, '1'); });
  ...
  it('clears state on drag end', () => {
    ...
    act(() => { result.current.handleDragStart(startEvent, '1'); });
    act(() => { result.current.handleDragEnd({} as React.DragEvent); });
    expect(result.current.draggedId).toBeNull();
  });
  ...
  it('sets dragOverId on drag over different element', () => {
    const { result } = renderHook(() => useDragAndDrop(vi.fn()));
    const startEvent = { dataTransfer: { effectAllowed: '', setData: vi.fn() } } as unknown as React.DragEvent;
    const overEvent = { preventDefault: vi.fn(), dataTransfer: { dropEffect: '' } } as unknown as React.DragEvent;
    act(() => { result.current.handleDragStart(startEvent, '1'); });
    act(() => { result.current.handleDragOver(overEvent, '2'); });
  ...
  it('does not set dragOverId when dragging over same element', () => {
    ...
    const overEvent = { preventDefault: vi.fn(), dataTransfer: { dropEffect: '' } } as unknown as React.DragEvent;
    act(() => { result.current.handleDragStart(startEvent, '1'); });
    act(() => { result.current.handleDragOver(overEvent, '1'); });
    expect(result.current.dragOverId).toBeNull();
  });
  ...
  it('reorders entries on drop', () => {
    ...
    act(() => { result.current.handleDragStart(startEvent, '1'); });
    act(() => { result.current.handleDrop(dropEvent, '2'); });
  ...
  it('does not reorder when dropping on same element', () => {
    ...
    const dropEvent = { preventDefault: vi.fn() } as unknown as React.DragEvent;
    act(() => { result.current.handleDragStart(startEvent, '1'); });
    act(() => { result.current.handleDrop(dropEvent, '1'); });
    expect(setEntries).not.toHaveBeenCalled();
  });

  it('handles setEntries callback correctly', () => {
    let entries = [...mockEntries];
    const setEntries = vi.fn().mockImplementation((fn) => { entries = fn(entries); });
    const { result } = renderHook(() => useDragAndDrop(setEntries));
    const startEvent = { dataTransfer: { effectAllowed: '', setData: vi.fn() } } as unknown as React.DragEvent;
    const dropEvent = { preventDefault: vi.fn() } as unknown as React.DragEvent;
    act(() => { result.current.handleDragStart(startEvent, '1'); });
