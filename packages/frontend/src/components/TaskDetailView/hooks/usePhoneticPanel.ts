...
export function usePhoneticPanel(
    ...
    if (!entry.stressedText) {
      try {
        ...
        if (analyzeResponse.ok) {
          ...
          if (stressedText) {
            setEntries(prev => prev.map(e => e.id === entryId ? { ...e, stressedText } : e));
          }
        }
      } catch (error) {
  ...
  const handlePhoneticApply = useCallback(async (newPhoneticText: string) => {
    ...
    setEntries(prev =>
      prev.map(entry => {
        if (entry.id === phoneticPanelEntryId) {
          return { ...entry, text: newPlainText, stressedText: newPhoneticText, audioUrl: null, audioBlob: null };
        }
        return entry;
      })
    );
    ...
    try {
      await DataService.getInstance().updateTaskEntry(userId, task.id, phoneticPanelEntryId, {
        text: newPlainText,
