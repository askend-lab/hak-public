...
export function usePronunciationVariants(
  ...
  const handleUseVariant = useCallback(async (variantText: string) => {
    if (selectedEntryId !== null && selectedTagIndex !== null && task) {
      ...
      setEntries(prev =>
        prev.map(entry => {
          if (entry.id === selectedEntryId) {
            return { ...entry, text: newText, stressedText: newStressedText, audioUrl: null, audioBlob: null };
          }
          return entry;
        })
      );
      ...
      try {
        if (userId) {
          await DataService.getInstance().updateTaskEntry(userId, task.id, selectedEntryId, {
            text: newText,
  ...
  }, [selectedEntryId, selectedTagIndex, task, entries, setEntries, userId, handleCloseVariants]);
