...
describe('UserProfile Full', () => {
  ...
  it('calls logout when logout clicked', async () => {
    ...
    const logout = vi.fn();
    vi.mocked(useAuth).mockReturnValue({ logout, user: null, isAuthenticated: false, isLoading: false, login: vi.fn(), showLoginModal: false, setShowLoginModal: vi.fn() });
  ...
  it('displays user initials', () => {
    render(<UserProfile {...props} />);
    // User avatar should show initials or icon
    expect(screen.getByText('Test User')).toBeInTheDocument();
  });

  it('handles user without name gracefully', () => {
    render(<UserProfile user={{ id: '123', name: '', email: 'a@b.c' }} />);
  });
});
