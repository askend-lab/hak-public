...
jest.mock('../ui', () => ({
  ...
  LogoWithText: () => <div data-testid="logo-with-text">LogoWithText</div>,
  NavTab: ({ label, ...props }: any) => <button data-testid="nav-tab" {...props}>{label}</button>,
  Button: ({ children, ...props }: any) => <button data-testid="button" {...props}>{children}</button>,
  WaffleMenu: () => <div data-testid="waffle-menu">WaffleMenu</div>,
  UserAvatar: ({ initials, ...props }: any) => <div data-testid="user-avatar" {...props}>{initials}</div>,
}));
