...
const mockFeature = (name: string, scenarios: { name: string; steps: string[] }[], tags: string[] = [], desc = '') => ({
  name, description: desc, tags, scenarios: scenarios.map(s => ({ ...s, tags: [] })),
});

const mockSuites = [
  { name: 'Suite1', status: 'passed' as const, tests: [
    { name: 'Scenario 1', fullName: 'Suite1 Scenario 1', status: 'passed' as const, duration: 123.45 },
  ]},
];
...
describe('SpecsContent test results', () => {
  ...
  it('shows skipped status for skipped features', () => {
    const feature = mockFeature('Feature', [{ name: 'Scenario', steps: [] }], ['@skip']);
    render(<SpecsContent feature={feature} testSuites={[]} />);
    expect(screen.getByText(/○ skipped/)).toBeInTheDocument();
  });

  it('shows pending status when no test result', () => {
    const feature = mockFeature('Feature', [{ name: 'Unknown Scenario', steps: [] }]);
    render(<SpecsContent feature={feature} testSuites={mockSuites} />);
...
describe('SpecsContent steps', () => {
  it('renders step keywords', () => {
    const feature = mockFeature('Feature', [{ name: 'Scenario', steps: ['Given something', 'When action', 'Then result'] }]);
    render(<SpecsContent feature={feature} testSuites={[]} />);
  ...
  it('renders And/But keywords', () => {
    const feature = mockFeature('Feature', [{ name: 'Scenario', steps: ['And more', 'But not this'] }]);
    render(<SpecsContent feature={feature} testSuites={[]} />);
  ...
  it('handles steps without keyword', () => {
    const feature = mockFeature('Feature', [{ name: 'Scenario', steps: ['plain step text'] }]);
    render(<SpecsContent feature={feature} testSuites={[]} />);
