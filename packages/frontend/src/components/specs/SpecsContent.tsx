...
export default function SpecsContent({ feature, testSuites }: SpecsContentProps) {
  ...
  return (
    <>
      ...
      {feature.description && <p className="specs-page__feature-desc">{feature.description}</p>}

      {feature.scenarios.map((scenario, sIdx) => {
        const result = getTestResult(testSuites, scenario.name);
        const status = feature.tags.includes('@skip') ? 'skipped' : (result?.status === 'passed' ? 'passed' : 'pending');
        return (
            ...
            {scenario.steps.map((step, stepIdx) => {
              ...
              return (
                <div key={stepIdx} className="specs-step">
                  <span className="specs-step__keyword">{match?.[1] ?? ''}</span>
                  {match?.[2] ?? step}
                </div>
              );
            })}
