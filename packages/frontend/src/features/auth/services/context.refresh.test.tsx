...
describe("Token Refresh", () => {
  ...
  it("should clear auth when refresh endpoint fails", async () => {
    const expiredToken = createMockJwt({ exp: Math.floor(Date.now() / 1000) - 100 });
    mockAuthStorage.getAccessToken.mockReturnValue(expiredToken);
  ...
  it("should call backend refresh endpoint with credentials", async () => {
    const expiredToken = createMockJwt({ exp: Math.floor(Date.now() / 1000) - 100 });
    mockAuthStorage.getAccessToken.mockReturnValue(expiredToken);
    global.fetch = vi.fn().mockResolvedValueOnce({
      ok: true, json: () => Promise.resolve({ access_token: "new-at", id_token: "new-id" }),
    });
    ...
    expect(fetchCall[0]).toContain("/tara/refresh");
