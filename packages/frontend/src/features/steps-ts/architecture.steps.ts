...
const extractPaths = (content: string) => [...content.matchAll(/^\s+path:\s+(\/\S+)/gm)].map((m) => m[1] ?? "").filter(Boolean);
...
function parseAuthorizers(slsPath: string): void {
  ...
  for (const line of lines) {
    ...
    if (am && curPath) { curAuth = am[1] ?? null; }
  }
  if (curPath) { state.authByRoute[curPath] = curAuth; }
}
...
When("rate limit rules are parsed", function () {
  ...
  state.rateLimits = [...state.wafContent.matchAll(re)].map((m) => ({ limit: parseInt(m[1] ?? "0", 10) }));
...
Given("the CloudWatch alarm definitions in Terraform", function () {
  ...
  state.alarmFns = [...content.matchAll(/FunctionName\s*=\s*"([^"]+)"/g)].map((m) => m[1] ?? "").filter(Boolean);
...
Given("the Lambda functions defined in serverless.yml files", function () {
  ...
  for (const slsPath of SLS_PATHS) {
    ...
    if (!fm?.[1]) { continue; }
    state.lambdaFns.push(...[...fm[1].matchAll(/^  (\w+):\s*$/gm)].map((m) => m[1] ?? "").filter(Boolean));
  }
