...
Then('I see the header', async function(this: TestWorld) {
  await this.waitFor(() => {
    return this.container?.querySelector('header, .header, [data-testid="header"]');
  });
});

Then('I see the footer', async function(this: TestWorld) {
  await this.waitFor(() => {
    return this.container?.querySelector('footer, .footer, [data-testid="footer"]');
  });
});

Then('I see a loading indicator', async function(this: TestWorld) {
  await this.waitFor(() => {
    return this.container?.querySelector('.loading, [class*="loading"], [class*="spinner"]');
  });
});
