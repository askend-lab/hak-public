...
When('I click play on the playlist', async function(this: TestWorld) {
  await this.waitFor(() => {
    const playButton = this.container?.querySelector('.playlist-play, [data-testid="playlist-play"]');
    if (playButton) {
...
When('I click pause', async function(this: TestWorld) {
  await this.waitFor(() => {
    const pauseButton = this.container?.querySelector('.pause, [data-testid="pause"]');
    if (pauseButton) {
...
When('I click next', async function(this: TestWorld) {
  await this.waitFor(() => {
    const nextButton = this.container?.querySelector('.next, [data-testid="next"]');
    if (nextButton) {
...
When('I click previous', async function(this: TestWorld) {
  await this.waitFor(() => {
    const prevButton = this.container?.querySelector('.previous, [data-testid="previous"]');
    if (prevButton) {
...
When('I add a sentence to the playlist', async function(this: TestWorld) {
  await this.waitFor(() => {
    const addButton = this.container?.querySelector('[data-testid="add-to-playlist"]');
    if (addButton) {
...
When('I remove an entry from the playlist', async function(this: TestWorld) {
  await this.waitFor(() => {
    const removeButton = this.container?.querySelector('[data-testid="remove-from-playlist"]');
    if (removeButton) {
...
When('I download the playlist', async function(this: TestWorld) {
  await this.waitFor(() => {
    const downloadButton = this.queryByText('Lae alla') || this.container?.querySelector('[data-testid="download-playlist"]');
    if (downloadButton) {
...
Then('the entry is removed from the playlist', async function(this: TestWorld) {
...
Then('I see {int} entries in the playlist', async function(this: TestWorld, count: number) {
  await this.waitFor(() => {
    const entries = this.container?.querySelectorAll('.playlist-entry, [data-testid="playlist-entry"]');
