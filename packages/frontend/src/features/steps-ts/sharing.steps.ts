...
When('I click the share button', async function(this: TestWorld) {
  await this.waitFor(() => {
    const shareButton = this.queryByText('Jaga') || 
...
When('I copy the share link', async function(this: TestWorld) {
  await this.waitFor(() => {
    const copyButton = this.queryByText('Kopeeri') || 
...
When('I click copy entries to my task', async function(this: TestWorld) {
  await this.waitFor(() => {
    const copyButton = this.queryByText('Kopeeri ülesandesse') ||
...
Then('I see the share modal', async function(this: TestWorld) {
  await this.waitFor(() => {
    return this.container?.querySelector('.share-modal, [data-testid="share-modal"]') ||
...
Then('I see a share link', async function(this: TestWorld) {
  await this.waitFor(() => {
    return this.container?.querySelector('input[readonly], .share-link') ||
...
Then('I can view the shared task', async function(this: TestWorld) {
  await this.waitFor(() => {
    return this.container?.querySelector('.task-detail, [data-testid="shared-task"]');
  });
});
...
Then('I see a success notification', async function(this: TestWorld) {
  await this.waitFor(() => {
    return this.container?.querySelector('.notification-success, [class*="success"]') ||
