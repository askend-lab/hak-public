...
When('I view the phonetic form', async function(this: TestWorld) {
  ...
  await this.waitFor(() => {
    return this.queryByTestId('phonetic-text') || this.container?.textContent?.match(/[`´'+]/);
  });
});
...
Then('the stressed syllables are visually distinct', async function(this: TestWorld) {
  await this.waitFor(() => {
    ...
    const stressedElements = container.querySelectorAll('[class*="stress"], [class*="accent"]');
...
Then('compound word boundaries are marked with {string}', async function(this: TestWorld, marker: string) {
  await this.waitFor(() => {
    const container = this.container;
    if (!container) return false;
    const text = container.textContent || '';
...
Then('I can see both original text and phonetic form', async function(this: TestWorld) {
  await this.waitFor(() => {
    ...
    const hasOriginal = container.querySelector('.original-text, [data-testid="original-text"]');
    const hasPhonetic = container.querySelector('.phonetic-text, [data-testid="phonetic-text"]');
    return hasOriginal || hasPhonetic || container.querySelectorAll('.tag').length > 0;
  });
});
...
Then('the differences are highlighted', async function(this: TestWorld) {
  await this.waitFor(() => {
    ...
    const highlighted = container.querySelectorAll('[class*="highlight"], [class*="diff"], .tag');
...
When('I click the save button', async function(this: TestWorld) {
  const saveButton = this.queryByText('Salvesta') || 
...
When('I click the reset button', async function(this: TestWorld) {
  const resetButton = this.queryByText('Lähtesta') || 
...
Then('a confirmation message appears', async function(this: TestWorld) {
  await this.waitFor(() => {
    return this.container?.querySelector('.notification, [class*="success"]') ||
