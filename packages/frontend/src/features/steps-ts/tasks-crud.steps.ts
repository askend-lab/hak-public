...
When("I navigate to the tasks section", async function (this: TestWorld) {
  const tasksNav = this.container?.querySelector(
    '[data-testid="tasks-nav"], nav a[href*="task"]',
  );
  if (tasksNav) {
    this.click(tasksNav);
  }
...
Then("I see my task list", async function (this: TestWorld) {
  await this.waitFor(() => {
    return this.container?.querySelector(
      '.task-list, [data-testid="task-list"]',
    );
  });
});
...
Then("I see task detail view", async function (this: TestWorld) {
  await this.waitFor(() => {
    return this.container?.querySelector(
      '.task-detail, [data-testid="task-detail"]',
    );
  });
});
...
Then("I see an edit button", async function (this: TestWorld) {
  await this.waitFor(() => {
    return (
      this.queryByText("Muuda") ||
      this.queryByText("Edit") ||
...
Then("the edit form opens", async function (this: TestWorld) {
  await this.waitFor(() => {
    return this.container?.querySelector(
      '.edit-modal, [data-testid="edit-modal"]',
    );
  });
});
...
Then("a confirmation dialog appears", async function (this: TestWorld) {
  await this.waitFor(() => {
    return this.container?.querySelector(
      '.confirm-dialog, [data-testid="confirm-dialog"]',
    );
  });
});
...
When("I cancel the deletion", async function (this: TestWorld) {
  const cancelButton =
    this.queryByText("Tühista") || this.queryByText("Cancel");
  if (cancelButton) {
