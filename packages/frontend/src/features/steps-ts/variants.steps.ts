...
Given('the variants panel is open', async function(this: TestWorld) {
  await this.waitFor(() => {
    return this.queryByTestId('variants-panel') || this.queryByText('Variandid');
  });
});
...
When('I click the play button next to a variant', async function(this: TestWorld) {
  await this.waitFor(() => {
    const playButton = this.container?.querySelector('.variant-play, [aria-label*="play"], button[class*="play"]');
    if (playButton) {
...
When('I view the variant list', async function(this: TestWorld) {
  ...
  await this.waitFor(() => {
    return this.container?.querySelector('.variant-list, [data-testid="variant-list"]');
  });
});

Then('the pronunciation variants panel opens', async function(this: TestWorld) {
  await this.waitFor(() => {
    return this.queryByTestId('variants-panel') || 
...
Then('I see multiple pronunciation options', async function(this: TestWorld) {
  await this.waitFor(() => {
    const variants = this.container?.querySelectorAll('.variant-item, [class*="variant"]');
...
Then('each variant has a description or context tag', async function(this: TestWorld) {
  await this.waitFor(() => {
    const descriptions = this.container?.querySelectorAll('.variant-description, .variant-context, [class*="description"]');
...
Then('each variant has a play button', async function(this: TestWorld) {
  await this.waitFor(() => {
    const playButtons = this.container?.querySelectorAll('.variant-play, [aria-label*="play"], button[class*="play"]');
