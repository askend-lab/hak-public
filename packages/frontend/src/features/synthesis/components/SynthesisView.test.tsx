...
import { createMockContext } from "./SynthesisView.test-helpers";

interface MockHeaderProps { onPlayAllClick: () => void }
interface MockItemProps {
  id: string; onPlay: (id: string) => void; onClear: (id: string) => void;
  menuContent: React.ReactNode; onMenuOpenLegacy: (e: React.MouseEvent, id: string) => void;
  onInputKeyDown?: (e: { key: string }) => void; onInputBlur?: () => void;
...
vi.mock("@/features/synthesis/components/SynthesisPageHeader", () => ({
  default: ({ onPlayAllClick }: MockHeaderProps) => (
    <div data-testid="header"><button onClick={onPlayAllClick}>Play All</button></div>
  ),
}));

vi.mock("@/features/synthesis/components/SentenceSynthesisItem", () => ({
  default: ({ id, onPlay, onClear, menuContent, onMenuOpenLegacy, onInputKeyDown, onInputBlur, tagMenuItems }: MockItemProps) => (
    <div data-testid={`item-${id}`}>
      <button onClick={() => onPlay(id)}>Play {id}</button>
      <button onClick={() => onClear(id)}>Clear {id}</button>
      <button onClick={(e: React.MouseEvent) => onMenuOpenLegacy(e, id)}>Menu {id}</button>
      {onInputKeyDown && <button onClick={() => onInputKeyDown({ key: "Enter" })}>KeyDown {id}</button>}
      ...
      {tagMenuItems?.map((item: { label: string; onClick: (id: string, idx: number, word: string) => void }) => (
        <button key={item.label} onClick={() => item.onClick(id, 0, "word")}>TagMenu-{item.label}</button>
      ))}
      {menuContent}
    </div>
  ),
...
vi.mock("@/features/synthesis/components/SentenceMenu", () => ({
  default: ({ sentenceId, onClose }: MockMenuProps) => (
    <div data-testid={`menu-${sentenceId}`}><button onClick={onClose}>Close Menu</button></div>
  ),
}));
...
describe("SynthesisView", () => {
  ...
  it("renders menu content when openMenuId matches and anchorEl exists", () => {
    ...
    mockContextValue = createMockContext({
      menu: { ...createMockContext().menu, openMenuId: "s1", menuAnchorEl: { s1: el } } as SynthesisPageContextValue["menu"],
    });
    render(<SynthesisView />);
  ...
  it("does not render menu when openMenuId does not match", () => {
    mockContextValue = createMockContext({
      menu: { ...createMockContext().menu, openMenuId: "s3" } as SynthesisPageContextValue["menu"],
    });
    render(<SynthesisView />);
