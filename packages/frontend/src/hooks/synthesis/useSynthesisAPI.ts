...
export function useSynthesisAPI(): {
  ...
  synthesizeText: (text: string, phoneticText?: string) => Promise<SynthesisResult>;
  synthesizeWithCache: (text: string, cachedPhoneticText?: string | null, cachedAudioUrl?: string | null) => Promise<SynthesisResult>;
} {
  ...
  const synthesizeText = useCallback(async (text: string, phoneticText?: string): Promise<SynthesisResult> => {
    ...
    if (phoneticText) {
      ...
      if (stressedText) {
        stressedTags = stressedText.trim().split(/\s+/).filter((w: string) => w.length > 0);
      }
    }

    const audioUrl = await synthesizeWithPolling(actualPhoneticText, getVoiceModel(actualPhoneticText));
  ...
  const synthesizeWithCache = useCallback(async (
    text: string,
