...
export function useSynthesisOrchestrator(): ReturnType<typeof useSentenceState> & {
  ...
  const { currentAudio, stopCurrentAudio, playAudio, playWithAbort } = audioPlayer;
  ...
  const playSingleSentence = useCallback(async (id: string, abortSignal?: AbortSignal, retryCount = 0): Promise<boolean> => {
    ...
    if (!audioUrl) {
      try {
        ...
        const result = await synthesisAPI.synthesizeWithCache(sentence.text, sentence.phoneticText, null);
  ...
  const synthesizeAndPlay = useCallback(async (id: string) => {
    ...
    if (sentence.audioUrl && sentence.phoneticText) {
      try {
        ...
        await playAudio(sentence.audioUrl, {
          ...
          onEnded: () => updateSentence(id, { isPlaying: false }),
          onError: () => {
            updateSentence(id, { isLoading: false, isPlaying: false, audioUrl: undefined, phoneticText: undefined });
    ...
    try {
      const result = await synthesisAPI.synthesizeText(sentence.text, sentence.phoneticText || undefined);
      ...
      await playAudio(result.audioUrl, {
        onLoadComplete: () => {
          ...
          if (result.stressedTags && result.stressedTags.length === tags.length) {
        ...
        onEnded: () => updateSentence(id, { isPlaying: false }),
        onError: () => updateSentence(id, { isLoading: false, isPlaying: false })
      });
    } catch (error) {
  ...
  const synthesizeWithText = useCallback(async (id: string, text: string) => {
    ...
    try {
      const result = await synthesisAPI.synthesizeText(text, sentence?.phoneticText || undefined);
