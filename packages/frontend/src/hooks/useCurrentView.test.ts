...
describe('useCurrentView', () => {
  ...
  it('returns synthesis view for /synthesis path', () => {
    mockUseLocation.mockReturnValue({ pathname: '/synthesis', search: '', hash: '', state: null, key: 'default' });
    const { result } = renderHook(() => useCurrentView());
    expect(result.current.currentView).toBe('synthesis');
  });

  it('returns tasks view for /tasks path', () => {
    mockUseLocation.mockReturnValue({ pathname: '/tasks', search: '', hash: '', state: null, key: 'default' });
    const { result } = renderHook(() => useCurrentView());
  ...
  it('returns tasks view with task id for /tasks/:id path', () => {
    mockUseLocation.mockReturnValue({ pathname: '/tasks/task-123', search: '', hash: '', state: null, key: 'default' });
    const { result } = renderHook(() => useCurrentView());
    expect(result.current.currentView).toBe('tasks');
  ...
  it('returns specs view for /specs path', () => {
    mockUseLocation.mockReturnValue({ pathname: '/specs', search: '', hash: '', state: null, key: 'default' });
    const { result } = renderHook(() => useCurrentView());
  ...
  it('returns role-selection view for /role-selection path', () => {
    mockUseLocation.mockReturnValue({ pathname: '/role-selection', search: '', hash: '', state: null, key: 'default' });
    const { result } = renderHook(() => useCurrentView());
