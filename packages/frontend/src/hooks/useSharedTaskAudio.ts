...
export function useSharedTaskAudio(): UseSharedTaskAudioReturn {
  ...
  const handlePlayEntry = useCallback((id: string, entries: TaskEntry[]) => {
  ...
  const playSingleEntry = useCallback(async (entry: TaskEntry, abortSignal: AbortSignal): Promise<boolean> => {
    ...
    try {
      ...
      return new Promise((resolve) => {
        ...
        audio.onerror = (): void => { cleanup(); resolve(false); };
  ...
  const handlePlayAll = useCallback(async (entries: TaskEntry[]) => {
    ...
    setCurrentPlayingId(null);
    setCurrentLoadingId(null);
  }, [isPlayingAll, isLoadingPlayAll, playAllAbortController, currentAudio, playSingleEntry]);
