...
const setupMocks = (): void => {
  ...
  global.fetch = vi.fn().mockResolvedValue({ ok: true, json: () => Promise.resolve({ stressedText: 'mock' }), blob: () => Promise.resolve(new Blob()) });
};

describe('useSynthesis core', () => {
  beforeEach(() => { vi.clearAllMocks(); localStorage.clear(); setupMocks(); });
  ...
  it('clears sentence content', () => {
    const { result } = renderHook(() => useSynthesis());
    const sentenceId = result.current.sentences[0]?.id ?? '';
    act(() => { result.current.handleTextChange(sentenceId, 'test'); });
  ...
  it('resets to initial sentence when removing last one', () => {
    const { result } = renderHook(() => useSynthesis());
    const sentenceId = result.current.sentences[0]?.id ?? '';
    act(() => { result.current.handleRemoveSentence(sentenceId); });
    expect(result.current.sentences).toHaveLength(1);
  ...
  it('opens and closes tag menu', () => {
    const { result } = renderHook(() => useSynthesis());
    const sentenceId = result.current.sentences[0]?.id ?? '';
    act(() => { result.current.setOpenTagMenu({ sentenceId, tagIndex: 0 }); });
  ...
  it('starts tag editing with handleEditTag', () => {
    ...
    expect(result.current.editingTag).toEqual({ sentenceId, tagIndex: 0, value: 'Noormees' });
  });
  ...
  it('updates and commits tag edit', () => {
    ...
    const sentenceId = result.current.sentences[0]?.id ?? '';
    act(() => { result.current.handleEditTag(sentenceId, 0); });
    act(() => { result.current.handleEditTagChange('NewValue'); });
  ...
  it('deletes tag from sentence', () => {
    ...
    act(() => { result.current.handleDeleteTag(sentenceId, 0); });
    expect(result.current.sentences[0]?.tags.length).toBe(initialCount - 1);
  });

  it('loads entries from localStorage on mount', () => {
    localStorage.setItem('eki_playlist_entries', JSON.stringify([{ id: 's1', text: 'Hello World', stressedText: 'Hello World' }]));
    const { result } = renderHook(() => useSynthesis());
  ...
  it('processes Enter key to add tags', () => {
    ...
    act(() => { result.current.handleKeyDown({ key: 'Enter', preventDefault: vi.fn() } as unknown as React.KeyboardEvent, sentenceId); });
  ...
  it('applies variant to specific tag', () => {
    ...
    act(() => { result.current.setDemoSentences(); });
    const sentenceId = result.current.sentences[0]?.id ?? '';
    act(() => { result.current.handleUseVariant('NewVariant', sentenceId, 0); });
  ...
  it('applies phonetic text to sentence', () => {
    ...
    act(() => { result.current.setDemoSentences(); });
    const sentenceId = result.current.sentences[0]?.id ?? '';
    act(() => { result.current.handleSentencePhoneticApply(sentenceId, 'test phonetic'); });
  ...
  it('handlePlay sets loading state', () => {
    ...
    expect(result.current.sentences[0]?.isLoading || result.current.sentences[0]?.isPlaying).toBeDefined();
  });
  ...
  it('handleDownload can be called without errors', async () => {
    ...
    act(() => { result.current.setDemoSentences(); });
    const sentenceId = result.current.sentences[0]?.id ?? '';
    await act(async () => { await result.current.handleDownload(sentenceId); });
  });
  ...
  it('handleEditTagKeyDown commits on Enter and cancels on Escape', () => {
    ...
    const sentenceId = result.current.sentences[0]?.id ?? '';
    act(() => { result.current.handleEditTag(sentenceId, 0); });
    act(() => { result.current.handleEditTagKeyDown({ key: 'Enter', preventDefault: vi.fn() } as unknown as React.KeyboardEvent); });
    expect(result.current.editingTag).toBeNull();
    act(() => { result.current.handleEditTag(sentenceId, 0); });
    act(() => { result.current.handleEditTagKeyDown({ key: 'Escape', preventDefault: vi.fn() } as unknown as React.KeyboardEvent); });
    expect(result.current.editingTag).toBeNull();
  });
});
