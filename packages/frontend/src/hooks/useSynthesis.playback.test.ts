...
vi.mock('@/utils/synthesize', () => ({ synthesizeWithPolling: (): Promise<string> => mockSynthesize() }));
...
describe('useSynthesis playback', () => {
  ...
  it('handlePlay synthesizes and plays audio', async () => {
    ...
    await act(async () => { await result.current.handlePlay(id); });
  });
  ...
  it('handlePlay stops other playing sentences', async () => {
    ...
    await act(async () => { await result.current.handlePlay(id1); });
    await act(async () => { await result.current.handlePlay(id2); });
  });

  it('handlePlayAll plays all sentences', async () => {
    const { result } = renderHook(() => useSynthesis());
    act(() => { result.current.setDemoSentences(); });
    await act(async () => { result.current.handlePlayAll(); });
    expect(result.current.isPlayingAll || result.current.isLoadingPlayAll).toBeDefined();
  });
  ...
  it('handleDeleteTag removes tag from sentence', () => {
    ...
    if (initialTagCount > 0) {
      ...
      expect(result.current.sentences[0]?.tags?.length).toBeLessThan(initialTagCount);
    }
  });
  ...
  it('handleEditTagChange updates editing value', () => {
    ...
    const id = result.current.sentences[0]?.id ?? '';
    act(() => { result.current.handleEditTag(id, 0); });
    act(() => { result.current.handleEditTagChange('new value'); });
  ...
  it('handleEditTagCommit saves changes', () => {
    ...
    const id = result.current.sentences[0]?.id ?? '';
    act(() => { result.current.handleEditTag(id, 0); });
    act(() => { result.current.handleEditTagChange('updated'); });
  ...
  it('handleEditTagKeyDown handles Enter key', () => {
    ...
    act(() => { result.current.handleEditTagKeyDown({ key: 'Enter', preventDefault } as unknown as React.KeyboardEvent); });
    expect(result.current.editingTag).toBeNull();
  });
  ...
  it('handleEditTagKeyDown handles Escape key', () => {
    ...
    act(() => { result.current.handleEditTag(id, 0); });
    const preventDefault = vi.fn();
    act(() => { result.current.handleEditTagKeyDown({ key: 'Escape', preventDefault } as unknown as React.KeyboardEvent); });
    expect(result.current.editingTag).toBeNull();
  });
  ...
  it('handleUseVariant updates sentence phonetic', () => {
    ...
    act(() => { result.current.setDemoSentences(); });
    const id = result.current.sentences[0]?.id ?? '';
    act(() => { result.current.handleUseVariant('new`phonetic', id, 0); });
  });
  ...
  it('handles synthesis error gracefully', async () => {
    ...
    act(() => { result.current.setDemoSentences(); });
    const id = result.current.sentences[0]?.id ?? '';
    await act(async () => { result.current.handlePlay(id); });
