...
jest.mock('./feature-parser', () => ({
  parseFeatureContent: jest.fn((): ParsedFeature => ({
    name: 'Test Feature',
...
describe('hooks', () => {
  ...
  describe('useFeatureData', () => {
    it('should fetch and parse feature data on mount', async () => {
      ...
      expect(result.current).toStrictEqual(mockParsedData);
    });
  ...
  describe('useExpandedState', () => {
    it('should initialize with empty set', () => {
      ...
      expect(result.current.expanded).toStrictEqual(new Set());
    ...
    it('should toggle item expansion', () => {
      ...
      expect(result.current.expanded).toStrictEqual(new Set(['item1']));
