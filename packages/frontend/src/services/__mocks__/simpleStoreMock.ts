  ...
  global.fetch = vi.fn(async (url: string, options?: RequestInit) => {
    ...
    if (path === '/api/get' || path === '/api/get-shared') {
      ...
      if (type === 'unlisted' && pk === 'tasks' && sk) {
        ...
        if (!task) {
          return { ok: true, json: async (): Promise<{ success: boolean; item: null }> => ({ success: true, item: null }) };
        }
        return { ok: true, json: async (): Promise<{ success: boolean; item: { data: Task } }> => ({ success: true, item: { data: task } }) };
      }
