...
describe('DataService Baseline Task Operations', () => {
  ...
  describe('baseline task operations', () => {
    ...
    it('updateTask creates copy when modifying baseline task', async () => {
      ...
      if (baselineTask) {
        const updated = await dataService.updateTask(mockUserId, baselineTask.id, { name: 'Modified' });
    ...
    it('deleteTask soft-deletes baseline task', async () => {
      ...
      if (baselineTask) {
        const result = await dataService.deleteTask(mockUserId, baselineTask.id);
        expect(result).toBe(true);
        
        const tasksAfter = await dataService.getUserTasks(mockUserId);
        expect(tasksAfter.find(t => t.id === baselineTask.id)).toBeUndefined();
      }
    });
  ...
  describe('updateTaskEntry', () => {
    it('updates entry in user task', async () => {
      ...
      const updated = await dataService.updateTaskEntry(mockUserId, task.id, entryId, {
        text: 'Updated',
    ...
    it('throws when updating non-existent entry', async () => {
      ...
      await expect(
        dataService.updateTaskEntry(mockUserId, task.id, 'non-existent', { text: 'Test' })
    ...
    it('handles updating added entry in baseline task', async () => {
      ...
      if (baselineTask) {
        const entries = await dataService.addTextEntriesToTask(mockUserId, baselineTask.id, ['Added Entry']);
        const addedEntryId = entries[0]?.id ?? '';
        
        const task = await dataService.getTask(baselineTask.id, mockUserId);
        const baselineEntryCount = task?.entries?.filter(e => !e.id.startsWith('entry_')).length ?? 0;
        
        if (baselineEntryCount < (task?.entries?.length ?? 0)) {
          const updated = await dataService.updateTaskEntry(mockUserId, baselineTask.id, addedEntryId, {
            text: 'Modified Added',
