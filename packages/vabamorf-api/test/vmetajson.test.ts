...
describe("vmetajson", () => {
  ...
  describe("initVmetajson", () => {
    it("should spawn vmetajson process with correct arguments", () => {
      ...
      expect(mockSpawn).toHaveBeenCalledWith("./vmetajson", ["--path=/dict"], {
  ...
  describe("analyze", () => {
    it("should throw if not initialized", async () => {
      await expect(analyze("test")).rejects.toThrow(
        "vmetajson process not initialized",
      );
    });
    ...
    it("should send JSON input to stdin", async () => {
      ...
      setTimeout(() => {
        mockProcess.stdout.emit(
          "data",
          Buffer.from('{"annotations":{"tokens":[]}}\n'),
        );
      }, 10);
